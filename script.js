...
const partidosHexagonal = [
  ...
  // 🔮 PRÓXIMAS FECHAS (imagen 2)
  // Puedes ajustar las fechas según el día real del partido.
  // Aquí las dejo como ejemplo seguidas una de otra.

  {
    fechaISO: "2025-11-21",
    fechaTexto: "Vie, 21/11",
    hora: "16:30",
    torneo: "LigaPro Ecuabet",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Separar jugados / próximos y ordenar
  const partidosJugados = partidosHexagonal
    .filter((p) => p.jugado)
    .sort((a, b) => new Date(b.fechaISO) - new Date(a.fechaISO)); // más reciente primero

  const partidosProximos = partidosHexagonal
    .filter((p) => !p.jugado)
    .sort((a, b) => new Date(a.fechaISO) - new Date(b.fechaISO)); // más cercano primero
...
function renderUltimoResultado(partidosJugados) {
  ...
  const ultimo = partidosJugados[0];
